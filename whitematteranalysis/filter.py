...
def preprocess(inpd, min_length_mm,
    ...
    print "Discarding fibers shorter than", min_length_mm, \
        "mm (= step size * npts =", step_size, "*", min_length_pts, ")"
    
    #print "Removing short fibers. Tractography step size: ",  step_size
    #print "Keeping fibers of minimum length (in points): ", min_length_pts
...
def downsample(inpd, output_number_of_lines, return_indices=False, preserve_point_data=False):
    ...
    # final line count
    #print "Number of lines selected:", outpd.GetNumberOfLines()
    if return_indices:
...
def symmetrize(inpd):
    ...
    print "Input number of points: ", lastidx
...
def remove_outliers(inpd, min_fiber_distance, n_jobs=2):
    ...
    if True:
        ...
        print "Number retained after outlier removal: ", num_fibers
...
def anisotropic_smooth(inpd, fiber_distance_threshold, points_per_fiber=30, n_jobs=2):
    ...
    while not converged:
        print "ITERATION:", iteration_count, "SUM FIBER COUNTS:", numpy.sum(numpy.array(curr_count))
        print "number indices", len(curr_indices)
        ...
        print "DISTANCE MIN:", distances_flat[pair_order[0]], \
        ...
        print "SUM FIBER COUNTS:", numpy.sum(numpy.array(curr_count)), "SUM DONE FIBERS:", numpy.sum(done)
        print "MAX COUNT:" , numpy.max(numpy.array(curr_count)), "AVGS THIS ITER:", number_averages
...
def anisotropic_smooth_OLDNOTUSED(inpd, max_fiber_distance, points_per_fiber=30, n_jobs=2):
    """ Average nearby fibers.
    
    The pairwise fiber distance matrix is computed, then fibers
    are averaged with their neghbors until an edge (>max_fiber_distance) is encountered.
    ...
    if USE_PARALLEL:
        ...
        distances = numpy.zeros((current_fiber_array.number_of_fibers, current_fiber_array.number_of_fibers))
        for lidx in fiber_indices:
            distances[lidx, :] = \
                similarity.fiber_distance(current_fiber_array.get_fiber(lidx), current_fiber_array, 0)
    ...
    print "SUM FIBER COUNTS:", numpy.sum(numpy.array(curr_count))
    
    # now perform smoothing
    iteration_count = 0
    # while there are still neighbors to be averaged
    while numpy.min(distances) < max_fiber_distance:
        iteration_count += 1
        print "ITERATION:", iteration_count, "DISTANCE MIN:", numpy.min(distances), "DISTANCE COUNT:", distances.shape

        
        next_fibers = list()
        next_count = list()
        done = numpy.zeros(len(curr_fibers))

        number_averages = 0

        unvisited = numpy.nonzero(done==0)[0]
        print unvisited.shape

        distances_sorted = numpy.sort(distances)
        
        # visit each fiber. should do this in order of smallest distance to combine.
        #for fidx in range(0, len(curr_fibers)):
        #unvisited = numpy.nonzero(done==0)[0]
        while len(unvisited) > 0:
            # find best one to combine now, sort each row (row is axis=1, this sorts last axis)
            # the transposing stuff gets the rows and columns that were unvisited only.
            # must be a better way to do this. it's not distances[unvisited, unvisited]
            #dist = numpy.sort(distances[unvisited].T[unvisited].T)
            dist = distances_sorted[unvisited]
            #print dist.shape
            #print len(unvisited)
            fidx = numpy.argsort(dist[:,0])[0]
            fidx = unvisited[fidx]
            #print fidx, done[fidx]
            
            assert not done[fidx]
            
            if not done[fidx]:
                done[fidx] += 1
                #dist = numpy.sort(distances[fidx,:])
                # find the nearest one that has not been visited already
                dist = numpy.sort(distances[fidx, unvisited])
                if dist[0] < max_fiber_distance:
                    idx_min = numpy.nonzero(dist[0]==distances[fidx,:])[0][0]
                    # weighted average of the fibers (depending on how many each one represents)
                    next_fibers.append((curr_fibers[fidx] * curr_count[fidx] + curr_fibers[idx_min] *curr_count[idx_min]) / (curr_count[fidx] + curr_count[idx_min]))
                    # this was the regular average
                    #next_fibers.append((curr_fibers[fidx] + curr_fibers[idx_min])/2)
                    next_count.append(curr_count[fidx] + curr_count[idx_min])
                    done[idx_min] += 1
                    number_averages += 1
                else:
                    # stop averaging this one, we hit an edge
                    next_fibers.append(curr_fibers[fidx])
                    next_count.append(curr_count[fidx])
            unvisited = numpy.nonzero(done==0)[0]
            #print "fibers left:", len(unvisited)
        ...
        # make sure we don't average it with itself
        for lidx in fiber_indices:
            distances[lidx,lidx] = numpy.inf
    ...
    return outpd, numpy.array(curr_count)
