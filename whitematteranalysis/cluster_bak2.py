...
def spectral(input_polydata, number_of_clusters=300,
    ...
    if use_nystrom:
        ...
        row_sum_1 = numpy.sum(A, axis=0) + numpy.sum(B.T, axis=0)
    ...
    atlas_e_val, atlas_e_vec = numpy.linalg.eigh(A)
    print '<cluster.py> Done calculating eigenvectors.'
    #print "<cluster.py> Eigenvalue range should be in -1 to 1:", atlas_e_val[0], atlas_e_val[-1]    
    # Check how well our chosen number of eigenvectors models the data
    power = numpy.cumsum(atlas_e_val[::-1]) / numpy.sum(atlas_e_val)
    print "<cluster.py> Power from chosen number of eigenvectors (", number_of_eigenvectors, ')', power[number_of_eigenvectors]
    print '<cluster.py> Top eigenvalues:', atlas_e_val[::-1][1:number_of_eigenvectors]
    ...
    if use_nystrom:
        ...
        V = numpy.concatenate((atlas_e_vec, \
                                  numpy.dot(numpy.dot(B.T, atlas_e_vec), \
                                                numpy.diag(numpy.divide(1.0, atlas_e_val)))))
        ...
        atlas_e_vec_norm = numpy.sum(numpy.multiply(V, V),0)
        V = numpy.divide(V, atlas_e_vec_norm)
        ...
        embed = atlas_e_vec[:, -number_of_eigenvectors - 2: -2]
        embed = numpy.divide(embed.T, atlas_e_vec[:, -1]).T
