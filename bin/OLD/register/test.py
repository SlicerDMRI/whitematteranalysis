...
#parallel_jobs = 101
parallel_jobs = 15
print 'Using three threads per CPU:', parallel_jobs
...
def test_cluster(input_poly_datas, register, scale, outdir, number_of_fibers=150,
    ...
    pd_centroids, count, pdclusters, cluster_num, cluster_count = \
        wma.filter.anisotropic_smooth(pd_out, scale_for_test, points_per_fiber=20, n_jobs=parallel_jobs)

    # number of non-singleton clusters
    len(numpy.nonzero(count>1)[0])
    # scale 20mm seems good, try 10, 20, and 30 in case.
    
    # return the registration information
    return pd_centroids, count, pdclusters, cluster_num, cluster_count 
