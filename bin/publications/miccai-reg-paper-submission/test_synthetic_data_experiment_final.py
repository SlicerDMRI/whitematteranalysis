...
calculated_transforms3 = list()
for subj in register._subjects:
    calculated_transforms3.append(subj.transform)
calculated_transforms3 = numpy.array(calculated_transforms3)
error3 = gold_standard + calculated_transforms3
#error[:,6:9] = numpy.multiply(gold_standard[:,6:9], calculated_transforms[:,6:9])
error3[:,6:9] = numpy.divide(1.0, gold_standard[:,6:9]) - calculated_transforms3[:,6:9]

print numpy.round((error3*100))/100.0

print numpy.sqrt(numpy.mean(numpy.multiply(error2,error2),0))
print numpy.sqrt(numpy.mean(numpy.multiply(error3,error3),0))

print "TIME ELAPSED:", elapsed4
