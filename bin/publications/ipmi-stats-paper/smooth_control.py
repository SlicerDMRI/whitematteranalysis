...
outdir = '/Users/odonnell/Dropbox/Coding/OUTPUTS/Dec2012/ipmi-smooth'

input_mask = "{0}/*.vtk".format(indir)
input_poly_datas = glob.glob(input_mask)

input_poly_datas = input_poly_datas[3:4]
...
for fname in input_poly_datas:
    ...
    for sigma in kernel_sigmas:
        # was: upper_thresh=40 (that's why sigma 20 looked so nice and not oversmoothed???)
    ...
    for sigma in kernel_sigmas:
        ...
        pd_to_render = wma.filter.mask(pd, density >6, density)
        ...
        ren = wma.render.render(pd_to_render, scalar_bar=True, scalar_range=[0,50])
