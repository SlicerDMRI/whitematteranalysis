...
if requested_atlas == 'ORG-800FC-100HCP' or requested_atlas == 'ORG-2000FC-100HCP':
    ...
    if not os.path.exists(downloaded_population_T1_file):
        ...
        print '<wm_download_org_atlas> Skip downloading: There is an existing \'100HCP-population-mean-T1.nii.gz\' in the output folder.',
        print ''
    if not os.path.exists(downloaded_population_T2_file):
        download_file(population_T2_url, downloaded_population_T2_file)
    else:
        print '<wm_download_org_atlas> Skip downloading: There is an existing \'100HCP-population-mean-T2.nii.gz\' in the output folder.',
        print ''
    if not os.path.exists(downloaded_population_b0_file):
        download_file(population_b0_url, downloaded_population_b0_file)
    else:
        print '<wm_download_org_atlas> Skip downloading: There is an existing \'100HCP-population-mean-b0.nii.gz\' in the output folder.',
        print ''
